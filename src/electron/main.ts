...
const openBlockingReminderWindow = (reminder: Reminder) => {
  if (reminderWindow) {
    return;
  }
...
ipcMain.handle(
  "create-reminder",
  (_event, payload: { text: string; delayMinutes: number }) => {
...
ipcMain.handle("delete-reminder", (_event, id: string) => {
...
ipcMain.handle("acknowledge-reminder", (_event, id: string) => {
